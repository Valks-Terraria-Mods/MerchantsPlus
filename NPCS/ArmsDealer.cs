    ...
    class ArmsDealer : GlobalNPC
    {
        ...
        public override void SetupShop(int type, Chest shop, ref int nextSlot)
        {
            if (type == NPCID.ArmsDealer) // Make sure we are only editing the arms dealer npc.
            {
                ...
                short[] npcs = {NPCID.ArmsDealer, NPCID.Clothier, NPCID.Cyborg, NPCID.Demolitionist, NPCID.Dryad, NPCID.DyeTrader, NPCID.GoblinTinkerer, NPCID.Mechanic, NPCID.Painter, NPCID.PartyGirl, NPCID.Pirate, NPCID.SantaClaus, NPCID.Steampunker, NPCID.Stylist, NPCID.DD2Bartender, NPCID.Truffle, NPCID.WitchDoctor, NPCID.Wizard};

                foreach (short npc in npcs) {
                    if (Main.npc[npc].UsesPartyHat())
                    {
                        if (Main.hardMode)
                        {
                            shop.item[1].SetDefaults(ItemID.PartyBullet);
                        }
                        else {
